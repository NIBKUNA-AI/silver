...
-- 3. The SAFE trigger (V5) - Minimalist
--    We remove ALL complex logic. Just minimal insert.
...
-- 4. Check ENUMs (Diagnostics)
--    If 'parent' is not in user_role, that's a huge problem.
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'user_role') THEN
        RAISE NOTICE 'User Role Enum is MISSING!';
    END IF;
END $$;
