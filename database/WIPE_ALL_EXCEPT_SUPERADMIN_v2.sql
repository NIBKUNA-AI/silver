...
-- Version: V2 (Handles auth.user_claims and other auth dependencies)
...
BEGIN
  ...
  -- 4. Delete Auth Dependencies (The source of the error)
  -- Claims
  DELETE FROM auth.user_claims 
  WHERE user_id IN (SELECT id FROM auth.users WHERE email != super_admin_email);
  
  -- MFA Factors (Just in case)
  ...
  -- Identities (Usually cascades, but safe to be explicit)
